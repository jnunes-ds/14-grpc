...
func (c *Course) FindByCategoryId(categodyId string) ([]Course, error) {
	rows, err := c.db.Query("SELECT id, name, description, category_id FROM courses WHERE category_id = $1", categodyId)
	if err != nil {
		return nil, err
	}
	defer func(rows *sql.Rows) {
